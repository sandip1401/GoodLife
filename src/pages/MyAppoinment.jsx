...
export default function MyAppoinment() {
  ...
  const handleDoctorClick = (docId) => {
    const doctorId = typeof docId === "object" ? docId._id : docId;
            ...
            return (
              ...
              >
                <div className="flex justify-center w-full md:w-auto cursor-pointer">
                  <img
                    onClick={() => handleDoctorClick(item.docId)}
                ...
                <div className="flex flex-col gap-2 w-full items-center md:w-auto md:items-end md:ml-auto md:mt-auto ">
                  ...
                  {item.isCompleted ? (
                    <button className="sm:min-w-48 py-1 px-3 border border-green-600 rounded text-green-600">
                      Appointment Completed
                    </button>
                  ) : item.cancelled ? (
                    <button className="sm:min-w-48 py-1 px-3 border border-red-500 rounded text-red-500">
                      Appointment Cancelled
                    </button>
                  ) : (
                    <button
                      onClick={() => cancelAppointment(item._id)}
                      className="min-w-[170px] border border-zinc-300 text-zinc-600 rounded px-3 cursor-pointer py-1 hover:scale-105 hover:bg-red-500 hover:text-white active:scale-100 transition-all"
                    >
                      Cancel appointment
                    </button>
                  )}
